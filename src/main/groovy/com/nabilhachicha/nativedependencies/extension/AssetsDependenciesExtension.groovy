...
class AssetsDependenciesExtension {
    final String CONFIGURATION_SEPARATOR = ":"
    final def classifiers = ['asstes']
    ...
    def artifact (String dep, Closure... enablePrefixClosure) {
        ...
        if (dependency.size() < 3 || dependency.size()>4) {
            ...
            classifiers.each {
                dependencies << new AssetsDep (dependency: dep + CONFIGURATION_SEPARATOR + it, shouldPrefixWithLib: addLibPrefixToArtifact)
            }

        } else {
            dependencies << new AssetsDep (dependency: dep, shouldPrefixWithLib: addLibPrefixToArtifact)
        }
    }
    ...
    def artifact (Map m, Closure... enablePrefixClosure) {
        ...
        if(!m.containsKey('classifier')) {
            classifiers.each {
                dependencies << new AssetsDep (dependency: temp + CONFIGURATION_SEPARATOR + it, shouldPrefixWithLib: addLibPrefixToArtifact)
            }
        } else {
            dependencies << new AssetsDep (dependency: temp + CONFIGURATION_SEPARATOR + m['classifier'], shouldPrefixWithLib: addLibPrefixToArtifact)
        }
    }
}
